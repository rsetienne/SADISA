...
#' model (pr) this is theta, phi and I. For the random-fission (rf) model this is theta and I. For the density-dependent model (dd) - which can also be interpreted as
...
{
  if(!((model[1] == 'pm' | model[1] == 'pr' | model[1] == 'dd' | model[1] == 'rf') & model[2] == 'dl'))
  {
    stop('Simulations for this metacommunity model is not implemented yet.'); #still needs to be implemented for density-dependence in the local community
  }
...
ms_sim <- function(parsmc,ii,qq,model)
{
  if(model[1] == 'pm')
  {
    ...
    if(model[1] == 'pr')
    {
      ...
      if(model[1] == 'dd')
      {
        ...
        if(model[1] == 'rf')
        {
          if(parsmc[1] < 0) stop('Parameters should be positive');
  ...
  es0 <- stats::integrate(f = ff, lower = 0, upper = upper_limit, rel.tol = 1e-9, abs.tol = 0)$value;
  nx <- stats::rpois(n = length(es0),es0);
  xs <- rep(0,nx);
  for(cnt in seq_len(nx))
  {
    prob <- stats::runif(1);
    ff2 <- function(x)
    {
    ...
    xs[cnt] <- stats::uniroot(f = ff2, interval = c(0, root_upper), extendInt = "upX", tol = .Machine$double.eps)$root;  
  }
...
rf_estot_int <- function(x,parsmc,ii,qq)
{
  ...
  for(cnt in 1:length(x)) {
    if(x[cnt] > 0) {
      y[cnt] <- -th * x[cnt] + log(-expm1(-iiqq * x[cnt]));  
    } else 
    {
      y[cnt] <- -Inf;
    }
